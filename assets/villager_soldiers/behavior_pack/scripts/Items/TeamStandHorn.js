    ...
    itemComponentRegistry.registerCustomComponent("fv:team_stand_horn", {
        onUse(event) {
            ...
            const nearbyEntities = player.dimension.getEntities({
                ...
                tags: [targetTag],
                propertyOptions: [
                    { propertyId: "fv:stay_mode", value: { equals: targetPropertyModeValue } }
                ]
            });
            ...
            for (const ent of nearbyEntities) {
                if (ent.isValid && ent.hasComponent("minecraft:is_tamed")) {
                    try {
                        ent.setProperty("fv:stay_mode", setPropertyToValue);
                    } catch (e) {
            ...
            // SỬA LỖI COMMAND: Dùng system.run + player.runCommand (Sync) để tránh lỗi API
            system.run(() => {
                if (player.isValid) {
                    try {
                        player.runCommand(
