...
const TYPE_BOW = "flintlock_pistol:bow";
const TYPE_EMPTY = "fv:flintlock_pistol_empty";
const TYPE_BULLET_ITEM = "fv:bullet";
const TYPE_BULLET_ENTITY = "fv:entity_bullet";

const COOLDOWN_CAT = "flintlock_pistol_empty";
...
function findAndRemoveOneBullet(player) {
    ...
    for (let i = 0; i < inv.size; i++) {
        ...
        if (it && it.typeId === TYPE_BULLET_ITEM) {
            // Kiểm tra số lượng trước khi thao tác
            if (it.amount > 1) {
                it.amount -= 1;
                inv.setItem(i, it);
            } else {
                inv.setItem(i, undefined); // Xóa hẳn slot nếu chỉ còn 1 viên
            }
            return true;
        }
    }
...
world.afterEvents.itemReleaseUse.subscribe(ev => {
    ...
    player.dimension.playSound("flintlock.shoot", head, { volume: 6 });
});
...
world.afterEvents.itemUse.subscribe(async ev => {
    ...
    // FIX LỖI 2: Chặn trừ đạn nếu đang nạp (cooldown > 0)
    ...
    player.playAnimation("animation.nguoimau.reload_flintlock_pistol", {
        blendOutTime: 0.3,
        stopExpression: "!query.equipped_item_any_tag('slot.weapon.mainhand', 'fv:flintlock_pistol_empty')"
    });
