...
const TYPE_REAPER_BOW = "golden_reaper:bow";
const TYPE_REAPER_EMPTY = "fv:golden_reaper_empty";
const TYPE_BULLET_ITEM = "fv:cartridge";
const TYPE_BULLET_ENTITY = "fv:entity_cartridge";

const COOLDOWN_CAT = "golden_reaper_empty";
...
world.afterEvents.itemReleaseUse.subscribe(ev => {
    ...
    if (bullet) {
        ...
        try { bullet.applyImpulse({ x: dir.x * 90, y: dir.y * 90, z: dir.z * 90 }); } catch { }
        bullet.triggerEvent("fv:golden_reaper");
    }
    player.dimension.playSound("reaper.shoot", head, { volume: 6 });
});
...
world.afterEvents.itemUse.subscribe(async ev => {
    ...
    player.playAnimation("animation.nguoimau.golden_reaper_reloading", {
        blendOutTime: 0.3,
        stopExpression: "!query.equipped_item_any_tag('slot.weapon.mainhand', 'fv:golden_reaper_empty')"
    });
