...
| ------- | --------- | ----------- | ------------------- | -------- |
| [`addon/gollem_addon/`](addon/gollem_addon/README.md) | Stone Golems | `addon:` | `1.20.10+` (v1.0.17) | Ativo; validacao in-game parcial |
| [`addon/villagers_addon/`](addon/villagers_addon/README.md) | Minerador de Tunel | `va:` | `1.21.130+` + Script API (v1.0.0) | MVP em codigo; in-game pendente |
| [`addon/villager_soldiers/`](addon/villager_soldiers/README.md) | Villager Soldiers (terceiros) | `fv:` | `1.21.130+` | Referencia; nao e produto proprio |
...
.github/workflows/     CI (JSON, estrutura, build .mcaddon)
```
...
- `addon/gollem_addon` — independente.
- `addon/villagers_addon` — soft-dependencia de **uso** no Villager Soldiers (`fv:villager_free_handle` para contratar). Carrega sozinho, mas o hire do minerador exige o Soldiers ativo.
- `addon/villager_soldiers` — pack de terceiros completo; usar como referencia ou instalar se for jogar com o complemento.

## CI e release

- `validate_json.yml` — valida JSONs nos addons.
- `validate-structure.yml` — manifests de `addon/gollem_addon` e `addon/villagers_addon`.
- `build-mcaddon.yml` — em push na `main` e tags `v*`: gera artifacts `stone_golems` + `villagers_addon` a partir de `addon/`; GitHub Release so em tags.
...
`addon/villager_soldiers/` e addon de terceiros (AnhemSteve). Licenca/uso comercial nao estao documentados aqui — tratar como referencia ate revisar direitos de uso.
